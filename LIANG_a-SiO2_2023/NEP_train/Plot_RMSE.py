...
text(-6.5, -10, 'RMSE = {0:4.2f} mev/atom'.format(loss[-1, 4]*1000), fontsize=13)
xlim([-12, 0])
ylim([-12, 0])
...
subplot(2, 2, 4)
set_fig_properties([gca()])
plot(virial_train[:, 1], virial_train[:, 0], 'o', c="C3", ms = 5, label="Train dataset")
plot(virial_test[:, 1], virial_test[:, 0],  'o', c="C8", ms = 5, label="Test dataset")
plot([-10, 10], [-10, 10], c = "grey", lw = 1)
text(-1.0, -2.5, 'RMSE = {0:4.2f} mev/atom'.format(loss[-1, 6]*1000), fontsize=13)
xlim([-4, 7])
ylim([-4, 7])
